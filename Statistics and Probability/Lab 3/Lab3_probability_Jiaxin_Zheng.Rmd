...
**A streak length of 1 means Kobe made 1 successful shot before missing the next one. There is 1 hit and 1 miss in a streak of 1, and streak length of 0 mean miss the shot**
...
**The typical streak length is 0 (it represent missed shots). The longest streak is 3.**
```{r}
kobe_streak <- data.frame (length = calc_streak(kobe_basket$shot))
ggplot(data = kobe_streak, aes(x = length)) +
  geom_bar(fill= "orange", color = "black")+
  labs(title= "The Distribution of Kobe’s Streak Lengths From the 2009 NBA Finals",
       x = "Streak Length",
       y = "Count")
...
```{r set-seed}
set.seed(456852)                 # make sure to change the seed
```
...
**The typical streak length for this simulated is 0 (it represent missed shots), The longest streak of baskets in 133 shouts is 6 **

```{r}
set.seed(456852)  

#Create sim_streak 45% 
sim_streak <- data.frame(length = calc_streak(sim_basket))
...
ggplot(data = sim_streak, aes(x = length)) +
  geom_bar(fill = "orange", color = "black") +
  labs(title = "Simulated Distribution of Streak Lengths with 45% Accuracy",
       x = "Streak Length",
       y = "Count") +
  theme_bw()
```
...
**Somewhat similar, the hits and misses will be affected due to the random variation, but because it is based on the 45% success rate, overall distributions should be similar. In addition, the typical streak length result for this simulated is 0, 1, and the longest streak could be 5 or 6 these is extreme outliers.**
...
****Kobe Bryant's Streak vs Simulated Streak: the typical streak length result for both Kobe Bryant's Streak and Simulated Streak is 0, 1 (means shorter streaks are common). The longest streak of Kobe Bryant are 3 or 4, and Simulated Streak's longest streak could be 5 or 6 (this is extreme outliers).****

****Any Evidence that the hot hand model fits Kobe's shooting patterns?****
****1. If Kobe's shooting are more frequent than the Simulated Streak, it may be the "hot hand" effect.     2. If Kobe's streak closely to the Simulated Streak, there is no strong evdience that the "hot hand"     molde fits to Kobe's shooting patterns.****
* * *
