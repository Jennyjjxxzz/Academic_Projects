---
title: 'Foundations for statistical inference - Confidence intervals_5B'
author: "Jiaxin Zheng"
output:
...
**Q1: In this sample 61.7% of the adults think cliamte change affects their local community.**
...
**Q2: I don't expect other student's answer same as mine but very slightly, because this is a random sampling. **
...
**Q3: 95% confidence mean we are 95% sure the true mean of the population or 95% sure the value in a range.**
...
**Q4: Yes, if the data is from the entire population and using bootstrapping(or other correct statistic methods) my neighbor's interval would have the same 95% to capture this value.**
...
**Q5: I would expect 95% to capture the true population mean, because if you calculate a 95% confidence interval, this means that 95% of the interval are from random samples, and about 5% of the intervals would not. **
...
**Q6: About 96% of the intervals captured the true population proportion, because of random sampling variability, not all confidence intervals will perfectly match the expected confidence level. **

```{r}
knitr::include_graphics("Capture.jpg", erro = FALSE)
```
...
**Q7: It like catching fish, a higher confidence level means that you want to be certain that your interval captures the true proportion. Higher confidence level is more wider, and lower confidence level is more narrower. (At a 90% confidence level, z* = 1.645. 95% confidence level z* = 1.96. 99% confidence level z* = 2.58.**
...
**Q8: I use low confidence level 60%, the distribution become very narrow.**
...
**Q9: It is still narrow when confidence level is low.**
...
**At a level 85% confidence level, the intervals are still narrower than 95%.**
...
**Q11: As sample size increases, width of the confidence intervals becomes narrow and increasing the precision. When sample size decreases, width of the confidence intervals becomes wider, bigger in uncertainty and less in the accuracy.**
...
**Q12: Increasing the number of bootstrap samples does not change the standard error. Increasing the number of bootstrap helps produce stable estimates of the confidence interval, and not affect the width of the confidence interval.**
