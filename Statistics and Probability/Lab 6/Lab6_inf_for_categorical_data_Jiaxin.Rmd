...
**Q2: 463/(6040+463)= 0.07119 is the proportion of people who have texted while driving every day in the past 30 days and never wear helmets.**
...
no_helmet <- yrbss %>%
  filter(!is.na(text_while_driving_30d) & 
                helmet_12m=="never") %>%
  mutate(text_ind = ifelse(text_while_driving_30d == 30, "everyday", "other")) %>%
  count(text_ind)
...
**0.0159 is the margin of error for the estimate of the proportion of non-helmet wearers that have texted while driving each day for the past 30 days based on this survey **
```{r}
n <- 1000
p <- no_helmet$n[1]/sum(no_helmet$n) 
...
```{r students who exercise more than once day}
exercise_time<- yrbss %>%
  filter(!is.na(physically_active_7d)) %>%
  mutate(exercise = ifelse(physically_active_7d <"1", "yes", "no"))
...
```{r}
exercise_time %>%
 specify(response = exercise, success = "yes") %>%
 generate(reps = 1000, type = "bootstrap") %>%
 calculate(stat = "prop") %>%
 get_ci(level = 0.95)
```
...
z <- 1.96
p <- sum(exercise_time$exercise == "yes")/n
se <- z * sqrt((p*(1-p))/n)
me <- z * se

me
```
**The 95% confidence interval for the probability that a student exercise more than once a week is between 0.156- 0.169. ME is 0.0123.**

```{r}
unique(yrbss$school_night_hours_sleep)
```

```{r students sleep 5 hours}
sleep_hour<- yrbss %>%
  filter(!is.na(school_night_hours_sleep)) %>%
  mutate(five_hours = ifelse(school_night_hours_sleep =="<5", "yes", "no"))
...
```{r}
sleep_hour %>%
 specify(response = five_hours, success = "yes") %>%
 generate(reps = 1000, type = "bootstrap") %>%
 calculate(stat = "prop") %>%
 get_ci(level = 0.95)
```
...
me
```
**The 95% confidence interval for the probability that a student sleep more than 8hours is between 0.0739- 0.0831. ME is 0.0131**
...
**Q5: The ME increases as the population proportion increases. The margin of error maximized when the population proportion is around 0.5. As the population proportion moves away from 0.5, the margin of erro decreases.**
...
**Q6: The sampling distribution is normal, the center appear to be around 0.1, spread between 0.03 and 0.14. *
...
**Q7: As p increase the center increase too. The shape becomes right- skewed.**

8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

**Q8: As n increase, the spread decreases.**
...
**HN Null hypothesis: There is no difference in strength training between students that sleep more than 10+ hours and those who don’t. **

**HA Alternative hypothesis: There is a difference in strength training between students that sleep more than 10+ hours and those who don’t..**
...
me
```
**Proportion of students training 7 days: 95 % confident that the proportion of students who will strength exercise is between 0.161 and 0.175 and me is 0.0128 **

```{r}
sleep_10 <- yrbss %>%
  filter(!is.na(school_night_hours_sleep)) %>%
  mutate(sleep_ten_hours = ifelse(school_night_hours_sleep == "10+", "yes", "no"))

sleep_10 %>%
  count(sleep_ten_hours) %>%
  mutate(p = n / sum(n))
```

```{r}
sleep_10 %>%
 specify(response = sleep_ten_hours, success = "yes") %>%
 generate(reps = 1000, type = "bootstrap") %>%
 calculate(stat = "prop") %>%
 get_ci(level = 0.95)
```
...
```{r}
n <- nrow(sleep_10)
p <- sum(sleep_10$sleep_ten_hours == "yes")/n
z <- 1.96
se <- z*sqrt((p*(1-p))/n)
...
me
```
**Proportion of students who sleeps 10+ hours: 95 % confident that the proportion of students who sleeps 10+ hours is between 0.0230 and 0.0283 and me is 0.005464 **
...
**Q10: There would be 5% chance of detecting a change.**
