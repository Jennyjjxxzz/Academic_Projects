...
**Q1: There are 1458 rows and 123 columns of data.**
```{r}
dim(hfi)
```
...
**Q2: The linear model would be a predict method to know the personal freedom score depending on the county's personal freedom expression control.**
```{r}
...
**Q3: When the pf_expression_control increases, the pf_score increases as well.**
...
**Q4: It is hard to choose two point and make a good job of minimizing the sum of squares by clicking, and what I got for sum of squares is 1060.329**
...
**Q5: The hf_score = 5.153687 + 0.349862 x pd_expression_control**
...
**Q6: If someone saw the least squares regression line and not the actual data, they would look at the intersection of X-axis, Y axis and regression line. And will estimate the pf_score to be around 7.7.**
**Use equation to checks out: 4.61707+ 0.49143 x 6.7= 7.909651 **
...
**Q7: There is no clear pattern in the residuals and they are randomly.**
...
**Q8: Yes, the histogram is approximately normally distributed. The qq-plot is close to a straight line.**
...
**Q10: When pf_relighion increase the pf_expression_control increase too.**

```{r}
ggplot(data = hfi, aes (x= pf_religion, y= pf_expression_control)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```
...
**Q11: The higher R^2 value indicates a stronger relationship between independent variable and the dependent variable.**
```{r}
m3 <- lm (pf_religion ~ pf_expression_control, data = hfi)
summary(m3)
```
...
**Q12: I am surprised between pf_religion and pf_identity_divorce**
```{r}
ggplot(data = hfi, aes (x= pf_religion, y= pf_identity_divorce)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```

```{r}
m4 <- lm (pf_religion ~ pf_identity_divorce, data = hfi)
summary(m4)
```
