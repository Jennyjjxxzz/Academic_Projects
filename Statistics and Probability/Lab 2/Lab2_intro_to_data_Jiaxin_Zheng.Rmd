...
**The binwidth is change, the large binwith value more clear for x values. The data values are the same.**
...
ggplot(sfo_feb_flights, aes(x = arr_delay)) +
  ...
  labs(title = "Histogram of Arrival Delays for Flights to SFO in February",
...
**AA & UA has the most variable arrival delays**
```{r}
...
**1. Lowest Mean Departure Delay: 
      pros: It provide an overall average both minor and major delays
      cons: It is not represent experienced by the most flights. (example: a few fights with long delays             can effect the overall means)
  2. Lowest Median Departure Delay: 
      pros: Know the middle values of the fights delays data, will not effect by the higher or lower                delays
      cons: Will ignores some large delays**
...
**I will choose LGA first, and second JFK**
...
ggplot(nycflights, aes (x = distance, y = avg_speed, colour = avg_speed))+
  geom_point(size = 3, alpha = 0.5)+
  geom_smooth(method = lm, se = F)+
  labs (title = "Avergae Speed and Distance", x = "Distance (miles)", y = "Average Speed (mph)")+
  theme_bw()
